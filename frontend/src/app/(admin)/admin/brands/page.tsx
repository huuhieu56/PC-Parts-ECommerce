...
export default function AdminBrandsPage() {
  ...
  const save = async () => {
    try {
      if (editId) { await api.put(`/brands/${editId}`, form); } else { await api.post("/brands", form); }
      setShowForm(false); setMsg({ type: "success", text: editId ? "Cập nhật thành công!" : "Tạo thương hiệu thành công!" }); fetchBrands();
  ...
  const remove = async (id: number) => {
    ...
    try { await api.delete(`/brands/${id}`); setBrands(prev => prev.filter(b => b.id !== id)); setMsg({ type: "success", text: "Đã xóa." }); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 flex items-center gap-1"><Plus className="w-4 h-4" /> Thêm thương hiệu</button>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-blue-200 p-6 mb-6">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">{editId ? "Sửa thương hiệu" : "Thêm thương hiệu mới"}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-full"><label className="block text-sm font-medium text-gray-700 mb-1">Mô tả</label>
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
          </div>
            ...
            <tbody>{filtered.map(b => (
              <tr key={b.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3"><div className="flex gap-1">
                  <button onClick={() => openEdit(b)} className="text-blue-600 hover:text-blue-700 p-1"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => remove(b.id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 className="w-4 h-4" /></button>
                </div></td>
              </tr>
            ))}</tbody>
          </table>
