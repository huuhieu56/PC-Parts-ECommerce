...
* *Nhân viên bán hàng (Sales): Quản lý đơn hàng, vận chuyển, xem thống kê doanh thu, xử lý bảo hành và đổi trả.*  
...
Hệ thống cần lưu trữ và xử lý 35 thực thể (Entities) chính đã triển khai, cùng 1 thực thể kế hoạch (Banner), được nhóm theo nghiệp vụ:
...
* ***Nhóm Bảo hành & Đổi trả:***
  ...
  * Chính sách đổi trả (Return\_Policy): *Gắn theo Category hoặc Product, số ngày đổi trả, cho phép đổi hàng/hoàn tiền, điều kiện, cờ loại trừ.*
  * Đổi trả (Return/Refund): *Order, Order\_Detail, lý do, loại (đổi hàng/hoàn tiền), trạng thái, số tiền hoàn, lý do từ chối, người xử lý.*
  * Ảnh minh chứng đổi trả (Return\_Image): *Ảnh Customer upload kèm yêu cầu đổi trả, lưu MinIO.*
* ***Nhóm Thông báo:***  
...
* ***Bảo hành & Đổi trả:***  
  ...
  * Category/Product \- Return\_Policy: *Gán chính sách đổi trả theo Category hoặc Product cụ thể. Ưu tiên Product.*
  ...
  * Return \- Return\_Image: *Một yêu cầu đổi trả có nhiều ảnh minh chứng (1-N).*
...
- Use Case Description: Cho phép Customer cập nhật thông tin cá nhân trong User/Profile, gồm họ tên, SĐT, ảnh đại diện, ngày sinh và giới tính. Chức năng này không bao gồm đổi mật khẩu (UC-CUS-16) hoặc quản lý địa chỉ giao hàng (UC-CUS-12).
- Actor: Khách hàng – Customer
- Priority: Chưa xác định (cần thống nhất với bảng yêu cầu người dùng).
...
2) Flow
- Basic Flow: Customer truy cập "Tài khoản của tôi" → "Thông tin cá nhân"; hệ thống hiển thị form với dữ liệu hiện tại gồm Họ tên, SĐT, ảnh đại diện, ngày sinh và giới tính; Customer chỉnh sửa thông tin, nhấn "Cập nhật"; hệ thống kiểm tra dữ liệu hợp lệ và lưu vào User/Profile.
- Alternative Flow: Customer có thể cập nhật ảnh đại diện riêng qua thao tác upload avatar. Nếu Customer muốn đổi mật khẩu hoặc quản lý địa chỉ, giao diện điều hướng sang UC-CUS-16 hoặc UC-CUS-12 tương ứng.
- Exception Flow: Nếu dữ liệu không hợp lệ thì hệ thống báo lỗi cụ thể theo từng trường; nếu SĐT đã được tài khoản khác sử dụng thì báo "SĐT đã được sử dụng"; nếu file ảnh đại diện sai định dạng hoặc vượt dung lượng cho phép thì báo lỗi upload; nếu Customer chưa đăng nhập hoặc token hết hạn thì yêu cầu đăng nhập lại.

3) Additional Information
- Business Rule:
  * Email thuộc Account chỉ hiển thị dạng chỉ đọc trong UC-CUS-14; thay đổi email không thuộc phạm vi use case này.
  * Họ tên là bắt buộc, độ dài 2-100 ký tự.
  * SĐT là bắt buộc, phải đúng định dạng số điện thoại Việt Nam 10 chữ số và là duy nhất trong User/Profile.
  * Ngày sinh là tùy chọn nhưng nếu nhập thì không được là ngày tương lai.
  * Giới tính là tùy chọn, chỉ nhận một trong các giá trị MALE, FEMALE, OTHER.
  * Ảnh đại diện là tùy chọn, chỉ nhận JPG/PNG/WebP và dung lượng tối đa 2MB.
- Non-Funtional Requirement: Áp dụng các yêu cầu phi chức năng tại Mục III (hiệu năng, bảo mật, khả dụng, dữ liệu, UX).
...
3) Additional Information
- Business Rule:
  * Thời hạn đổi trả được tính từ thời điểm Order chuyển sang trạng thái "Hoàn thành" (Order.completed_at hoặc mốc Order_Status_History tương ứng).
  * Linh kiện PC lắp ráp mới (CPU, Mainboard, RAM, VGA, SSD/HDD, PSU, Case, tản nhiệt) được yêu cầu đổi trả trong 30 ngày nếu lỗi kỹ thuật do nhà sản xuất.
  * Nhóm thiết bị công nghệ/phụ kiện/màn hình/laptop/thiết bị ngoại vi được yêu cầu đổi trả trong 15 ngày nếu lỗi kỹ thuật do nhà sản xuất.
  * Không áp dụng đổi trả với sản phẩm bán nguyên seal đã bóc seal, sản phẩm tiêu hao, phần mềm/license, quà tặng khuyến mãi, sản phẩm Apple hoặc sản phẩm hãng có chính sách riêng; các trường hợp này chuyển sang xử lý bảo hành nếu đủ điều kiện.
  * Sản phẩm phải còn đầy đủ hộp, phụ kiện, sách hướng dẫn, phiếu bảo hành, hóa đơn/chứng từ nếu có; tem bảo hành, serial, mã vạch phải còn nguyên vẹn và đọc được.
  * Không tiếp nhận đổi trả nếu sản phẩm bị trầy xước nặng, móp méo, nứt vỡ, cháy nổ, vào nước, ố vàng, mốc, hoen rỉ, lỗi do sai điện áp, thiên tai, côn trùng, vận chuyển/lắp đặt sai quy cách hoặc đã bị tháo/sửa bởi bên không được ủy quyền.
  * Chỉ cho phép tạo một yêu cầu đổi trả đang ở trạng thái PENDING_APPROVAL hoặc APPROVED cho cùng Order_Detail.
  * Ảnh minh chứng là tùy chọn khi Customer tạo yêu cầu, nhưng nếu upload thì chỉ nhận JPG/PNG/WEBP, tối đa 5 ảnh, mỗi ảnh tối đa 5MB.
  * Nếu duyệt hoàn tiền, hệ thống tạo Payment status Refunded và ghi Inventory_Log loại RETURN. Nếu duyệt đổi hàng, hệ thống hoàn kho sản phẩm cũ và tạo Order mới cho sản phẩm thay thế.
- Non-Funtional Requirement: Áp dụng các yêu cầu phi chức năng tại Mục III (hiệu năng, bảo mật, khả dụng, dữ liệu, UX).
...
| UR-PROF-01 | Quản lý Người dùng | Quản lý thông tin cá nhân | Customer sửa User/Profile (họ tên, SĐT, ảnh đại diện, ngày sinh, giới tính). Đổi mật khẩu thuộc UC-CUS-16. | Trung bình |
