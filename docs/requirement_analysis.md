...
\-        Admin nhấn "Thêm banner", nhập: tiêu đề, upload hình ảnh (MinIO), URL liên kết (trang sản phẩm, danh mục, khuyến mãi), vị trí hiển thị (MAIN/SIDE_1/SIDE_2/SIDE_3/POPUP/EVENT/CUSTOM), thứ tự hiển thị, ngày bắt đầu, ngày kết thúc, trạng thái Active/Inactive
...
-        Vị trí banner (MAIN/SIDE_1/SIDE_2/SIDE_3/POPUP/EVENT) đã có banner khác → Báo lỗi

-        Vị trí hiển thị không hợp lệ → Báo lỗi

8\.    Hậu điều kiện: Banner được tạo/sửa/xóa trong CSDL. Hình ảnh được lưu trữ trên MinIO

9\.    Dữ liệu và API tham chiếu:

-        Entity chính: Banner (tiêu đề, image_url, link_url, placement, sort_order, start_date, end_date, status, created_at, updated_at)

-        Public API: `GET /api/v1/banners` chỉ trả banner `ACTIVE`, còn hiệu lực theo ngày bắt đầu/kết thúc, sắp xếp theo `sort_order` ASC, `created_at` DESC
