...
sequenceDiagram
    ...
    BE->>BE: Validate title, image type/size, date range
...
**Public query rule:** `GET /api/v1/banners` chỉ trả banner `ACTIVE`, có `start_date` rỗng hoặc `<= NOW()`, có `end_date` rỗng hoặc `>= NOW()`, sắp xếp theo `sort_order ASC`.
