...
| 1.4       | 2026-04-25 | —       | Bổ sung Test Cases: M01 (mật khẩu, profile, địa chỉ), M04 (vận chuyển, edge case trạng thái), M06 (nhà cung cấp), M10 (bổ sung). Thêm mới M11 (quản trị tài khoản, thống kê doanh thu) |
| 1.5       | 2026-04-30 | —       | Bổ sung thiết kế UC-CUS-12 (quản lý địa chỉ giao hàng), chuẩn hóa API Address, validation/business rules, cập nhật Checkout validate Address và tách test case TC-ADDR |
...
| 18a | Hình 3.15a | Sequence Diagram — Quản lý địa chỉ giao hàng             | UML Sequence   | 3.2.6.1 |
...
| 38  | Hình 3.35 | Sequence Diagram — Admin quản lý banner / slider         | UML Sequence   | 3.2.26 |
| 39  | Hình 3.36 | ERD — Nhóm Phân quyền (Auth & User)                      | ER Diagram     | 3.3.1  |
| 40  | Hình 3.37 | ERD — Nhóm Sản phẩm (Product Catalog)                    | ER Diagram     | 3.3.2  |
| 41  | Hình 3.38 | ERD — Nhóm Mua sắm & Đơn hàng (Shopping & Order)         | ER Diagram     | 3.3.3  |
| 42  | Hình 3.39 | ERD — Nhóm Tương tác & Bảo hành (Interaction & Warranty) | ER Diagram     | 3.3.4  |
| 43  | Hình 3.40 | ERD — Nhóm Thông báo (Notification)                      | ER Diagram     | 3.3.5  |
| 44  | Hình 3.41 | ERD — Nhóm Nội dung (Banner / Slider)                    | ER Diagram     | 3.3.6  |
...
class ReturnPolicy {
    ...
    - conditions : String
    - description : String
    + isEligible(completedAt) : Boolean
}
...
ReturnRequest --> Account : resolved by
ReturnRequest "1" --> "*" ReturnImage : has evidence
ReturnPolicy --> Category
...
**Phạm vi UC-CUS-14:** Customer chỉ cập nhật User/Profile gồm `fullName`, `phone`, `avatarUrl`, `dateOfBirth`, `gender`. Email thuộc Account được hiển thị chỉ đọc. Đổi mật khẩu thuộc UC-CUS-16; quản lý địa chỉ giao hàng thuộc UC-CUS-12.

**API chính:**

| Method | Endpoint | Mục đích | Payload / Response chính |
| :-- | :-- | :-- | :-- |
| GET | `/api/v1/users/me` | Lấy thông tin profile hiện tại | Trả `email`, `fullName`, `phone`, `avatarUrl`, `dateOfBirth`, `gender` |
| PUT | `/api/v1/users/me` | Cập nhật profile | Nhận `fullName`, `phone`, `dateOfBirth`, `gender`; trả `updatedProfile` |
| POST | `/api/v1/users/me/avatar` | Upload ảnh đại diện | Nhận `multipart/form-data`; trả `avatarUrl` |
...
| `phone` | Bắt buộc, đúng định dạng số điện thoại Việt Nam 10 chữ số, duy nhất trong User/Profile |
| `dateOfBirth` | Tùy chọn, không được là ngày tương lai |
| `gender` | Tùy chọn, chỉ nhận `MALE`, `FEMALE`, `OTHER` |
| `avatar` | Tùy chọn, chỉ nhận JPG/PNG/WebP, dung lượng tối đa 2MB |

#### 3.2.6.1 Luồng Quản lý địa chỉ giao hàng (UC-CUS-12)
...
| Method | Endpoint | Mục đích | Payload / Response chính |
| :-- | :-- | :-- | :-- |
| GET | `/api/v1/users/addresses` | Lấy danh sách địa chỉ của user hiện tại | Trả `AddressResponse[]`, sắp xếp `isDefault DESC`, `updatedAt DESC` |
| POST | `/api/v1/users/addresses` | Thêm địa chỉ mới | Nhận `AddressRequest`; trả `AddressResponse`; Address đầu tiên tự động là mặc định |
| PUT | `/api/v1/users/addresses/{id}` | Sửa địa chỉ | Nhận `AddressRequest`; chỉ cho sửa Address thuộc user hiện tại |
| DELETE | `/api/v1/users/addresses/{id}` | Xóa địa chỉ | Trả `204`; không cho xóa Address duy nhất |
| PATCH | `/api/v1/users/addresses/{id}/default` | Đặt địa chỉ mặc định | Trả `AddressResponse`; Address cũ của cùng user chuyển `isDefault=false` |

**AddressRequest:**

| Trường | Kiểu | Bắt buộc | Ghi chú |
| :-- | :-- | :-- | :-- |
| `label` | string | Không | Nhãn như `Nhà`, `Cơ quan`; tối đa 50 ký tự |
| `receiverName` | string | Có | Tên người nhận |
| `receiverPhone` | string | Có | SĐT người nhận |
| `province` | string | Có | Hiện tại chỉ hỗ trợ Hà Nội theo shipping restriction |
| `district` | string | Có | Quận/Huyện/Thị xã thuộc vùng giao hàng hỗ trợ |
| `ward` | string | Có | Phường/Xã |
| `street` | string | Có | Số nhà, tên đường |
| `isDefault` | boolean | Không | Chỉ dùng như ý định của client; Service vẫn phải đảm bảo mỗi user chỉ có 1 default |
...
| `isDefault` | boolean | Cờ địa chỉ mặc định |
...
| `receiverPhone` | Bắt buộc, đúng định dạng số điện thoại Việt Nam 10 chữ số |
| `province` | Bắt buộc; hiện tại chỉ nhận `Hà Nội` |
| `district` | Bắt buộc; phải thuộc danh sách quận/huyện/thị xã Hà Nội được hỗ trợ trong SDD |
| `ward` | Bắt buộc, 2-100 ký tự |
| `street` | Bắt buộc, 5-255 ký tự |

**Business Rules:**

| Mã rule | Nội dung |
| :-- | :-- |
| ADDR-BR-01 | Mọi Address bắt buộc thuộc user đang đăng nhập. Customer không được xem/sửa/xóa/đặt mặc định Address của user khác. |
| ADDR-BR-02 | Mỗi user có tối đa một Address `isDefault=true`. |
| ADDR-BR-03 | Address đầu tiên của user tự động được đặt `isDefault=true` dù request không truyền `isDefault`. |
| ADDR-BR-04 | Khi đặt một Address làm mặc định, hệ thống cập nhật Address đó `isDefault=true` và toàn bộ Address khác của cùng user `isDefault=false` trong cùng transaction. |
| ADDR-BR-05 | Không cho xóa Address duy nhất của user; trả lỗi "Bạn cần tạo địa chỉ mới trước khi xóa". |
| ADDR-BR-06 | Nếu Address đã được gắn với Order, không được hard delete làm mất lịch sử đơn hàng. Delete trong sổ địa chỉ phải dùng soft delete/ẩn khỏi danh sách hoặc bị từ chối cho đến khi có trường snapshot địa chỉ trên Order. |
| ADDR-BR-07 | Checkout chỉ chấp nhận `addressId` tồn tại, thuộc user hiện tại và còn khả dụng trong sổ địa chỉ. |
| ADDR-BR-08 | Address dùng cho Checkout phải nằm trong vùng giao hàng đang hỗ trợ. Hiện tại vùng hỗ trợ là Hà Nội theo SDD. |
...
| Thiếu/sai field bắt buộc | 400 | Trả validation errors theo field |
...
**Address validation trong Checkout:** Trước khi validate Coupon và kiểm tra tồn kho, OrderService phải kiểm tra `addressId` là bắt buộc, tồn tại, thuộc Customer đang đăng nhập và nằm trong vùng giao hàng hỗ trợ. Nếu Customer thêm địa chỉ mới từ Checkout, Frontend gọi luồng UC-CUS-12 (`POST /api/v1/users/addresses`) trước, sau đó dùng `id` của Address vừa tạo để gọi `POST /api/v1/orders`.

**Lỗi liên quan Address:**

| Tình huống | HTTP | Message |
| :-- | :-- | :-- |
| Không truyền `addressId` | 400 | `Vui lòng chọn địa chỉ giao hàng` |
| `addressId` không tồn tại hoặc không thuộc Customer hiện tại | 404 | `Không tìm thấy địa chỉ giao hàng` |
...
Luồng này bao phủ UC-CUS-11 (Customer tạo yêu cầu đổi trả) và UC-AD-10 (Admin/Sales xử lý). Customer chỉ được tạo yêu cầu khi Order thuộc về chính Customer, Order ở trạng thái `COMPLETED`, Order_Detail thuộc Order, sản phẩm còn trong hạn theo ReturnPolicy và chưa có yêu cầu đổi trả đang xử lý cho cùng Order_Detail.
...
#### 3.2.26 Luồng Admin — Quản lý banner / slider trang chủ

> **Hình 3.35** — UC-AD-11: Admin quản lý banner / slider trang chủ.
...
sequenceDiagram
    ...
    Admin->>FE: Mở /admin/banners
    FE->>BE: GET /api/v1/admin/banners
    BE->>DB: SELECT banner ORDER BY sort_order
    DB-->>BE: Danh sách banner
    BE-->>FE: BannerDto[]

    Admin->>FE: Thêm/Sửa banner + chọn ảnh
    FE->>BE: POST/PUT multipart /api/v1/admin/banners
    BE->>BE: Validate title, image type/size, date range, placement (hợp lệ & duy nhất nếu không phải CUSTOM)
    BE->>MinIO: Upload image vào folder banners
    MinIO-->>BE: image_url
    BE->>DB: INSERT/UPDATE banner
    BE-->>FE: BannerDto

    Admin->>FE: Kéo thả sắp xếp
    FE->>BE: PATCH /api/v1/admin/banners/reorder
    BE->>DB: UPDATE sort_order theo danh sách id/order
    BE-->>FE: BannerDto[]
```

### 3.3 Sơ đồ quan hệ thực thể (ERD)

Hệ thống quản lý **35 thực thể** chính, được chia thành 5 nhóm nghiệp vụ.
...
ERD nhóm này bao gồm ReturnPolicy, ReturnRequest và ReturnImage để lưu chính sách đổi trả, yêu cầu đổi trả theo Order_Detail và ảnh minh chứng của Customer.
...
#### 3.3.6 ERD — Nhóm Nội dung (Banner / Slider)

UC-AD-11 sử dụng entity độc lập `Banner`. Banner không phụ thuộc Product/Coupon vì URL liên kết có thể trỏ tới sản phẩm, danh mục, khuyến mãi hoặc nội dung marketing khác.

| Cột | Kiểu dữ liệu | Ràng buộc | Mô tả |
| :-- | :-- | :-- | :-- |
| id | BIGSERIAL | PK | Khóa chính |
| title | VARCHAR(255) | NOT NULL | Tiêu đề banner |
| image_url | VARCHAR(500) | NOT NULL | URL ảnh đã upload MinIO |
| link_url | VARCHAR(500) | NULLABLE | URL khi người dùng click |
| placement | VARCHAR(20) | NOT NULL | Vị trí hiển thị (MAIN/SIDE_1/SIDE_2/SIDE_3/POPUP/EVENT/CUSTOM) |
| sort_order | INT | DEFAULT 0 | Thứ tự hiển thị |
| status | VARCHAR(20) | DEFAULT 'ACTIVE' | `ACTIVE` hoặc `INACTIVE` |
| start_date | TIMESTAMP | NULLABLE | Ngày bắt đầu hiển thị |
| end_date | TIMESTAMP | NULLABLE | Ngày kết thúc hiển thị |
| created_at | TIMESTAMP | DEFAULT NOW() | Ngày tạo |
| updated_at | TIMESTAMP | DEFAULT NOW() | Ngày cập nhật |

**Public query rule:** `GET /api/v1/banners` chỉ trả banner `ACTIVE`, có `start_date` rỗng hoặc `<= NOW()`, có `end_date` rỗng hoặc `>= NOW()`, sắp xếp theo `sort_order ASC`, `created_at DESC`.
...
| TC-AUTH-11 | Đổi mật khẩu thành công                 | Customer đã đăng nhập                     | 1. PUT /api/v1/users/password với oldPwd đúng, newPwd hợp lệ             | Trả 200, password_hash cập nhật                                | P    |
| TC-AUTH-12 | Đổi mật khẩu — mật khẩu cũ sai          | Customer đã đăng nhập                     | 1. PUT /api/v1/users/password với oldPwd sai                              | Trả 400, "Mật khẩu hiện tại không chính xác"                   | N    |
| TC-AUTH-13 | Đổi mật khẩu — mật khẩu mới trùng cũ    | Customer đã đăng nhập                     | 1. PUT /api/v1/users/password với newPwd = oldPwd                         | Trả 400, "Mật khẩu mới phải khác mật khẩu cũ"                 | N    |
| TC-AUTH-14 | Đổi mật khẩu — xác nhận không khớp      | Customer đã đăng nhập                     | 1. PUT /api/v1/users/password với newPwd ≠ confirmPwd                     | Trả 400, "Mật khẩu xác nhận không khớp"                       | N    |
| TC-AUTH-15 | Quên mật khẩu với email tồn tại         | Account có email `user@example.com`       | 1. Gọi POST /auth/forgot-password với email tồn tại                       | Trả 200, tạo Token RESET_PASSWORD còn hạn, gửi email reset    | P    |
| TC-AUTH-16 | Quên mật khẩu với email không tồn tại   | Email không tồn tại trong hệ thống        | 1. Gọi POST /auth/forgot-password với email không tồn tại                 | Trả 200, message trung tính "Nếu email tồn tại..."            | N    |
| TC-AUTH-17 | Quên mật khẩu với email sai định dạng   | —                                         | 1. Gọi POST /auth/forgot-password với email sai định dạng                 | Trả 400, báo lỗi field email                                  | N    |
| TC-AUTH-18 | Quên mật khẩu nhiều lần liên tiếp       | Account tồn tại                           | 1. Gọi POST /auth/forgot-password 2 lần liên tiếp                         | Trả 200 cho cả 2 lần, token cũ vô hiệu, token mới có hiệu lực | P    |
| TC-AUTH-19 | Đặt lại mật khẩu thành công             | Có token RESET_PASSWORD hợp lệ            | 1. Gọi POST /auth/reset-password với token + newPassword + confirmPassword| Trả 200, cập nhật password, xóa token, xóa phiên bản cũ       | P    |
| TC-AUTH-20 | Đặt lại mật khẩu với token hết hạn      | Token RESET_PASSWORD đã hết hạn           | 1. Gọi POST /auth/reset-password với token hết hạn                        | Trả 400, message "Liên kết đã hết hạn..."                     | N    |
| TC-AUTH-21 | Đặt lại mật khẩu với token không hợp lệ | Token không tồn tại hoặc sai format       | 1. Gọi POST /auth/reset-password với token không hợp lệ                   | Trả 400, thông báo token không hợp lệ/hết hạn                 | N    |
| TC-AUTH-22 | Đặt lại mật khẩu với xác nhận không khớp| Token hợp lệ                              | 1. Gọi POST /auth/reset-password với confirm khác newPassword             | Trả 400, message "Mật khẩu xác nhận không khớp"               | N    |
| TC-AUTH-23 | Đặt lại mật khẩu với mật khẩu yếu       | Token hợp lệ                              | 1. Gọi POST /auth/reset-password với mật khẩu yếu                         | Trả 400, message báo rule độ mạnh mật khẩu                    | N    |
| TC-AUTH-24 | Token cũ bị vô hiệu sau khi reset       | User đã reset mật khẩu thành công         | 1. Gọi API bảo vệ bằng access/refresh token cũ                            | Trả 401, buộc đăng nhập lại                                   | P    |
| TC-AUTH-25 | Đặt lại mật khẩu thiếu trường bắt buộc  | Token hợp lệ                              | 1. Gọi POST /auth/reset-password thiếu dữ liệu                            | Trả 400, errors chỉ rõ field thiếu                            | N    |
| TC-AUTH-26 | Cập nhật profile thành công             | Customer đã đăng nhập                     | 1. PUT /api/v1/users/me với fullName, phone, dateOfBirth, gender hợp lệ                      | Trả 200, User/Profile cập nhật                                | P    |
| TC-AUTH-27 | Cập nhật profile — SĐT sai định dạng | Customer đã đăng nhập                     | 1. PUT /api/v1/users/me với phone sai định dạng                                   | Trả 400, validation error field phone                                     | N    |
| TC-AUTH-28 | Cập nhật profile — SĐT đã tồn tại | Customer đã đăng nhập, phone thuộc user khác | 1. PUT /api/v1/users/me với phone đã tồn tại | Trả 409, message "SĐT đã được sử dụng" | N |
| TC-AUTH-29 | Cập nhật profile — ngày sinh tương lai | Customer đã đăng nhập | 1. PUT /api/v1/users/me với dateOfBirth lớn hơn ngày hiện tại | Trả 400, validation error field dateOfBirth | N |
| TC-AUTH-30 | Cập nhật profile — giới tính không hợp lệ | Customer đã đăng nhập | 1. PUT /api/v1/users/me với gender ngoài MALE/FEMALE/OTHER | Trả 400, validation error field gender | N |
| TC-AUTH-31 | Upload avatar thành công | Customer đã đăng nhập, file PNG/JPG/WebP <= 2MB | 1. POST /api/v1/users/me/avatar với file hợp lệ | Trả 200, avatarUrl được cập nhật trong User/Profile | P |
| TC-AUTH-32 | Upload avatar — sai định dạng | Customer đã đăng nhập | 1. POST /api/v1/users/me/avatar với file PDF | Trả 400, validation error file type | N |
| TC-AUTH-33 | Upload avatar — vượt dung lượng | Customer đã đăng nhập | 1. POST /api/v1/users/me/avatar với file > 2MB | Trả 400, validation error file size | N |
| TC-AUTH-34 | Cập nhật profile khi chưa đăng nhập | Chưa có access token hợp lệ | 1. PUT /api/v1/users/me với dữ liệu hợp lệ | Trả 401, yêu cầu đăng nhập lại | N |
| TC-AUTH-40 | Đăng nhập email sai định dạng           | —                                         | 1. POST /auth/login với email "not-an-email"                              | Trả 400, validation error                                     | N    |
| TC-AUTH-41 | Refresh Token hết hạn                   | Refresh token đã hết hạn                  | 1. POST /auth/refresh-token                                               | Trả 400/401, yêu cầu đăng nhập lại                            | N    |

#### 3.4.1.1 Module Quản lý địa chỉ giao hàng (UC-CUS-12)

| TC-ID | Tên test case | Điều kiện tiên quyết | Các bước thực hiện | Kết quả mong đợi | Loại |
| :-- | :-- | :-- | :-- | :-- | :-- |
| TC-ADDR-01 | Xem danh sách địa chỉ | Customer đã đăng nhập, có ≥ 2 Address | 1. GET /api/v1/users/addresses | Trả 200, chỉ gồm Address của user hiện tại, Address mặc định đứng trước | P |
| TC-ADDR-02 | Thêm địa chỉ đầu tiên | Customer đã đăng nhập, chưa có Address | 1. POST /api/v1/users/addresses với dữ liệu hợp lệ | Trả 201, Address tạo thành công và `isDefault=true` | P |
| TC-ADDR-03 | Thêm địa chỉ mới | Customer đã đăng nhập, đã có Address mặc định | 1. POST /api/v1/users/addresses với đầy đủ thông tin | Trả 201, Address tạo thành công | P |
| TC-ADDR-04 | Thêm địa chỉ thiếu trường bắt buộc | Customer đã đăng nhập | 1. POST /api/v1/users/addresses thiếu `province` | Trả 400, validation error field `province` | N |
| TC-ADDR-05 | Thêm địa chỉ — SĐT người nhận sai định dạng | Customer đã đăng nhập | 1. POST /api/v1/users/addresses với `receiverPhone` sai format | Trả 400, validation error field `receiverPhone` | N |
| TC-ADDR-06 | Thêm địa chỉ ngoài vùng giao hàng | Customer đã đăng nhập | 1. POST /api/v1/users/addresses với `province` ngoài Hà Nội | Trả 400, "Địa chỉ nằm ngoài vùng giao hàng hỗ trợ" | N |
| TC-ADDR-07 | Sửa địa chỉ | Address id=1 thuộc user hiện tại | 1. PUT /api/v1/users/addresses/1 với `street` mới | Trả 200, Address cập nhật | P |
| TC-ADDR-08 | Sửa địa chỉ của user khác | Address id=1 thuộc user khác | 1. PUT /api/v1/users/addresses/1 | Trả 404, "Không tìm thấy địa chỉ" | N |
| TC-ADDR-09 | Đặt địa chỉ mặc định | User có ≥ 2 Address | 1. PATCH /api/v1/users/addresses/2/default | Trả 200, Address 2 `isDefault=true`, Address cũ `isDefault=false` | P |
| TC-ADDR-10 | Đặt mặc định Address không tồn tại | Address id không tồn tại | 1. PATCH /api/v1/users/addresses/999/default | Trả 404, "Không tìm thấy địa chỉ" | N |
| TC-ADDR-11 | Xóa địa chỉ không phải duy nhất | User có ≥ 2 Address | 1. DELETE /api/v1/users/addresses/1 | Trả 204, Address không còn hiển thị trong sổ địa chỉ | P |
| TC-ADDR-12 | Xóa địa chỉ duy nhất | User chỉ có 1 Address | 1. DELETE /api/v1/users/addresses/1 | Trả 400, "Bạn cần tạo địa chỉ mới trước khi xóa" | N |
| TC-ADDR-13 | Xóa địa chỉ của user khác | Address id=1 thuộc user khác | 1. DELETE /api/v1/users/addresses/1 | Trả 404, "Không tìm thấy địa chỉ" | N |
| TC-ADDR-14 | Gọi API khi chưa đăng nhập | Không có access token hợp lệ | 1. GET /api/v1/users/addresses | Trả 401, yêu cầu đăng nhập lại | N |
...
| TC-ID     | Tên test case             | Điều kiện tiên quyết                     | Các bước thực hiện                                                                 | Kết quả mong đợi                                                            | Loại |
| :-------- | :------------------------ | :--------------------------------------- | :--------------------------------------------------------------------------------- | :-------------------------------------------------------------------------- | :--- |
| TC-ORD-01 | Checkout COD thành công   | Giỏ có ≥ 1 item, có Address thuộc user, SP còn hàng | 1. POST /api/v1/orders với `paymentMethod=COD`, `addressId` hợp lệ | Trả 201. Order, OrderDetail, Payment, Shipping tạo. Inventory trừ. Cart xóa | P    |
| TC-ORD-02 | Checkout MoMo thành công  | Tương tự TC-ORD-01                       | 1. POST /api/v1/orders với `paymentMethod=MOMO`, `addressId` hợp lệ | Trả 201 + paymentUrl. Sau callback: Payment.status=SUCCESS                  | P    |
| TC-ORD-03 | Checkout khi SP hết hàng  | Inventory.quantity=0                     | 1. POST /api/v1/orders | Trả 409, liệt kê SP hết hàng. Order không tạo                               | N    |
| TC-ORD-04 | Áp dụng Coupon hợp lệ     | Coupon còn hạn, chưa max_uses            | 1. POST /api/v1/orders với `couponCode` | discount_amount tính đúng. Coupon_Usage tạo                                 | P    |
| TC-ORD-05 | Áp dụng Coupon hết hạn    | end_date < NOW()                         | 1. POST /api/v1/orders với `couponCode` | Trả 400, message "Mã giảm giá đã hết hạn"                                   | N    |
| TC-ORD-06 | Áp dụng Coupon đã dùng    | Coupon_Usage đã có record                | 1. POST /api/v1/orders với `couponCode` | Trả 400, message "Bạn đã sử dụng mã này"                                    | N    |
...
| TC-ORD-09 | Xem lịch sử đơn hàng      | Customer có ≥ 3 đơn                      | 1. GET /api/v1/orders                                                                     | Trả 200, chỉ đơn của user. Có pagination                                    | P    |
...
| TC-ORD-11 | Cập nhật thông tin vận chuyển         | Role SALES, Order CONFIRMED                | 1. PUT /admin/orders/1/shipping với provider, tracking, fee    | Trả 200, Shipping cập nhật                                   | P    |
| TC-ORD-12 | Chuyển trạng thái shipping — đã giao  | Shipping status=DELIVERING                 | 1. PATCH /admin/orders/1/shipping/status DELIVERED             | Shipping DELIVERED. Order tự chuyển COMPLETED. History ghi nhận | P    |
| TC-ORD-13 | Shipping thiếu mã vận đơn            | tracking_number trống                      | 1. PATCH shipping status=DELIVERING khi chưa có tracking       | Trả 400, "Vui lòng nhập mã vận đơn"                          | N    |
| TC-ORD-14 | Chuyển trạng thái không hợp lệ       | Order status=COMPLETED                     | 1. PATCH /admin/orders/1/status SHIPPING                       | Trả 400, "Không thể chuyển từ COMPLETED sang SHIPPING"       | N    |
| TC-ORD-15 | Hủy đơn — trạng thái không cho phép  | Order status=COMPLETED                     | 1. POST /api/v1/orders/1/cancel                                       | Trả 400, "Không thể hủy đơn đã hoàn thành"                   | N    |
| TC-ORD-16 | Checkout giỏ hàng trống              | Cart không có item                         | 1. POST /api/v1/orders                                                | Trả 400, "Giỏ hàng trống"                                    | N    |
| TC-ORD-17 | Checkout không có địa chỉ            | Customer không truyền `addressId`          | 1. POST /api/v1/orders không có `addressId`                            | Trả 400, "Vui lòng chọn địa chỉ giao hàng"                  | N    |
| TC-ORD-18 | Xem chi tiết đơn của người khác      | Order thuộc User A, User B đăng nhập       | 1. GET /api/v1/orders/1                                                | Trả 403 hoặc 404                                             | N    |
| TC-ORD-19 | Checkout với Address của user khác   | `addressId` thuộc User A, User B đăng nhập | 1. POST /api/v1/orders với `addressId` của User A                      | Trả 404, "Không tìm thấy địa chỉ giao hàng". Order không tạo | N    |
| TC-ORD-20 | Checkout với Address ngoài vùng giao hàng | Address không thuộc vùng Hà Nội hỗ trợ | 1. POST /api/v1/orders với `addressId` ngoài vùng hỗ trợ               | Trả 400, "Địa chỉ nằm ngoài vùng giao hàng hỗ trợ". Order không tạo | N |
...
| TC-INV-07 | Tạo nhà cung cấp                      | Role ADMIN                             | 1. POST /admin/suppliers với đầy đủ thông tin       | Trả 201, Supplier tạo thành công                        | P    |
| TC-INV-08 | Sửa nhà cung cấp                      | Supplier id=1 tồn tại                  | 1. PUT /admin/suppliers/1 với phone mới             | Trả 200, Supplier cập nhật                              | P    |
| TC-INV-09 | Xóa NCC đang được gắn vào Inventory   | Supplier id=1 liên kết với Inventory   | 1. DELETE /admin/suppliers/1                        | Trả 400, "Không thể xóa NCC đang được sử dụng"         | N    |
| TC-INV-10 | Xóa NCC không liên kết                | Supplier id=2 không liên kết           | 1. DELETE /admin/suppliers/2                        | Trả 204, Supplier xóa thành công                        | P    |
| TC-INV-11 | Nhập hàng số lượng ≤ 0                 | Role WAREHOUSE                         | 1. POST /admin/inventory/import qty=0               | Trả 400, "Số lượng phải lớn hơn 0"                      | N    |
| TC-INV-12 | Xem lịch sử biến động kho             | Product có ≥ 3 Inventory_Log           | 1. GET /admin/inventory/1/logs                      | Trả 200, danh sách log có pagination, mới nhất trước     | P    |
| TC-INV-13 | Gắn Supplier cho Inventory            | Supplier và Product tồn tại            | 1. PATCH /admin/inventory/1 với supplier_id         | Trả 200, Inventory.supplier cập nhật                    | P    |
...
| TC-WAR-08 | Từ chối đổi trả                 | Return PENDING_APPROVAL                   | 1. PATCH status=REJECTED, rejectionReason="Lý do"   | REJECTED. Lý do lưu. Không hoàn kho/tiền               | N    |
| TC-WAR-09 | Đổi trả khi đơn chưa hoàn thành | Order status=SHIPPING                     | 1. POST /returns                                    | Trả 400, "Chỉ đổi trả đơn đã hoàn thành"               | N    |
| TC-WAR-10 | Đổi trả quá hạn                 | Order COMPLETED, quá return_window_days   | 1. POST /returns                                    | Trả 400, "Đã quá thời hạn đổi trả cho sản phẩm này"    | N    |
| TC-WAR-11 | Đổi trả thiếu lý do             | Order COMPLETED, còn hạn                  | 1. POST /returns reason rỗng                        | Trả 400, "Vui lòng nhập lý do đổi trả"                 | N    |
| TC-WAR-12 | Đổi trả đơn không thuộc user    | Order thuộc user khác                     | 1. POST /returns                                    | Trả 403, không tạo Return                               | N    |
| TC-WAR-13 | Order_Detail không thuộc Order  | order_detail_id thuộc đơn khác            | 1. POST /returns                                    | Trả 400, "Sản phẩm không thuộc đơn hàng này"            | N    |
| TC-WAR-14 | Sản phẩm không áp dụng đổi trả  | ReturnPolicy.excluded = true              | 1. POST /returns                                    | Trả 400, "Sản phẩm này không áp dụng đổi trả"           | N    |
| TC-WAR-15 | Ảnh minh chứng sai định dạng    | Upload file không phải JPG/PNG/WEBP       | 1. POST multipart /returns                          | Trả 400, báo lỗi định dạng ảnh                          | N    |
| TC-WAR-16 | Tạo trùng yêu cầu đổi trả       | Đã có Return PENDING_APPROVAL cùng item   | 1. POST /returns cùng order_detail_id               | Trả 409, "Sản phẩm này đã có yêu cầu đổi trả đang xử lý" | N    |
...
| TC-NTF-08 | Gửi email thất bại vẫn tạo in-app notification | SMTP service không khả dụng              | 1. Trigger thay đổi trạng thái đơn hàng   | Email gửi thất bại, log ghi nhận. Notification in-app vẫn tạo | N    |
| TC-NTF-09 | Xóa thông báo                                  | Notification id=1 thuộc user hiện tại    | 1. DELETE /notifications/1                | Trả 204, notification bị xóa                                 | P    |

#### 3.4.11 Module Quản trị Hệ thống & Thống kê (M11)

> Test case cho quản lý tài khoản (UC-AD-06) và thống kê doanh thu (UC-AD-05).

| TC-ID     | Tên test case                             | Điều kiện tiên quyết                       | Các bước thực hiện                                              | Kết quả mong đợi                                                | Loại |
| :-------- | :---------------------------------------- | :----------------------------------------- | :-------------------------------------------------------------- | :--------------------------------------------------------------- | :--- |
| TC-ADM-01 | Xem danh sách tài khoản                  | Role ADMIN, DB có ≥ 5 Account              | 1. GET /admin/users                                             | Trả 200, danh sách Account+User có pagination                   | P    |
| TC-ADM-02 | Lọc tài khoản theo Role                  | DB có ≥ 2 Role khác nhau                   | 1. GET /admin/users?role=SALES                                  | Trả 200, chỉ Account có role SALES                              | P    |
| TC-ADM-03 | Tạo tài khoản nội bộ                     | Role ADMIN                                 | 1. POST /admin/users với email, password, role=WAREHOUSE        | Trả 201, Account (role=WAREHOUSE) + User tạo                    | P    |
| TC-ADM-04 | Tạo tài khoản — email trùng              | Email đã tồn tại                           | 1. POST /admin/users với email đã có                            | Trả 409, "Email đã tồn tại"                                     | N    |
| TC-ADM-05 | Khóa tài khoản                           | Account id=2, is_active=true               | 1. PATCH /admin/users/2/status is_active=false                  | Trả 200, is_active=false, Token/Session bị xóa (buộc đăng xuất) | P    |
| TC-ADM-06 | Mở khóa tài khoản                        | Account id=2, is_active=false              | 1. PATCH /admin/users/2/status is_active=true                   | Trả 200, is_active=true, cho phép đăng nhập lại                 | P    |
| TC-ADM-07 | Admin tự khóa chính mình                 | Admin đang đăng nhập                       | 1. PATCH /admin/users/{self_id}/status is_active=false          | Trả 400, "Không thể khóa tài khoản của chính mình"              | N    |
| TC-ADM-08 | Gán Role cho tài khoản                   | Account id=3 hiện là CUSTOMER              | 1. PATCH /admin/users/3/role role=SALES                         | Trả 200, Account.role cập nhật thành SALES                      | P    |
| TC-ADM-09 | Customer truy cập quản lý tài khoản      | Role = CUSTOMER                            | 1. GET /admin/users                                             | Trả 403 Forbidden                                               | N    |
| TC-ADM-10 | Sales truy cập quản lý tài khoản         | Role = SALES                               | 1. GET /admin/users                                             | Trả 403 Forbidden                                               | N    |
| TC-ADM-11 | Thống kê doanh thu — mặc định            | Có ≥ 5 Order COMPLETED trong tháng hiện tại | 1. GET /admin/reports/revenue                                   | Trả 200, tổng doanh thu, số đơn, số SP bán                      | P    |
| TC-ADM-12 | Thống kê doanh thu — lọc theo thời gian  | Có Order từ tháng 1 đến tháng 3            | 1. GET /admin/reports/revenue?from=2026-01-01&to=2026-03-31    | Trả 200, chỉ tổng hợp Order trong khoảng thời gian              | P    |
| TC-ADM-13 | Thống kê doanh thu — lọc theo Category   | Có Order chứa SP thuộc Category "CPU"      | 1. GET /admin/reports/revenue?category=1                        | Trả 200, doanh thu chỉ từ SP thuộc category_id=1                | P    |
| TC-ADM-14 | Top sản phẩm bán chạy                    | Có ≥ 10 SP đã bán                          | 1. GET /admin/reports/top-products                              | Trả 200, danh sách SP sắp xếp theo số lượng bán giảm dần        | P    |
| TC-ADM-15 | Thống kê — khoảng thời gian không hợp lệ | —                                          | 1. GET /admin/reports/revenue?from=2026-06-01&to=2026-01-01    | Trả 400, "Khoảng thời gian không hợp lệ"                        | N    |
| TC-ADM-16 | Thống kê — không có dữ liệu             | Không có Order trong khoảng thời gian      | 1. GET /admin/reports/revenue?from=2030-01-01&to=2030-12-31    | Trả 200, tổng doanh thu=0, số đơn=0                             | P    |
| TC-ADM-17 | Doanh thu theo danh mục                  | Có Order thuộc ≥ 3 Category khác nhau      | 1. GET /admin/reports/revenue-by-category                       | Trả 200, bảng doanh thu breakdown theo Category                 | P    |

#### 3.4.12 Module Nội dung (M12)

| TC-ID | Tên test case | Điều kiện tiên quyết | Các bước thực hiện | Kết quả mong đợi | Loại |
| :-- | :-- | :-- | :-- | :-- | :-- |
| TC-BAN-01 | Public xem banner active | Có ≥ 2 banner ACTIVE còn hiệu lực | 1. GET /api/v1/banners | Trả 200, chỉ banner active/còn hiệu lực, sắp xếp theo sort_order | P |
| TC-BAN-02 | Admin xem danh sách banner | Role ADMIN | 1. GET /api/v1/admin/banners | Trả 200, có cả ACTIVE/INACTIVE | P |
| TC-BAN-03 | Admin tạo banner thành công | Role ADMIN, ảnh JPG/PNG/WEBP ≤ 5MB | 1. POST multipart /api/v1/admin/banners | Trả 201, Banner tạo thành công, ảnh lưu MinIO | P |
| TC-BAN-04 | Tạo banner thiếu tiêu đề | Role ADMIN | 1. POST title rỗng | Trả 400, báo thiếu trường bắt buộc | N |
| TC-BAN-05 | Tạo banner thiếu ảnh | Role ADMIN | 1. POST không có image | Trả 400, báo thiếu ảnh | N |
| TC-BAN-06 | Ảnh sai định dạng | Role ADMIN, file GIF/PDF | 1. POST multipart image sai type | Trả 400, chỉ chấp nhận JPG/PNG/WEBP | N |
| TC-BAN-07 | Ảnh quá 5MB | Role ADMIN | 1. POST image > 5MB | Trả 400, ảnh tối đa 5MB | N |
| TC-BAN-08 | Ngày kết thúc trước ngày bắt đầu | Role ADMIN | 1. POST/PUT end_date < start_date | Trả 400, "Ngày kết thúc phải sau ngày bắt đầu" | N |
| TC-BAN-09 | Admin sửa banner | Role ADMIN, banner tồn tại | 1. PUT /api/v1/admin/banners/{id} | Trả 200, thông tin cập nhật | P |
| TC-BAN-10 | Admin xóa banner | Role ADMIN, banner tồn tại | 1. DELETE /api/v1/admin/banners/{id} | Trả 200/204, banner bị xóa | P |
| TC-BAN-11 | Admin sắp xếp banner | Role ADMIN, có ≥ 3 banner | 1. PATCH /api/v1/admin/banners/reorder | Trả 200, sort_order cập nhật theo danh sách | P |
| TC-BAN-12 | Customer truy cập admin banner | Role CUSTOMER | 1. GET /api/v1/admin/banners | Trả 403 Forbidden | N |
| TC-BAN-13 | Trùng vị trí banner non-custom | Role ADMIN, đã có banner MAIN/SIDE/POPUP/EVENT | 1. POST/PUT placement trùng | Trả 400, báo vị trí đã tồn tại | N |
| TC-BAN-14 | Vị trí banner không hợp lệ | Role ADMIN | 1. POST/PUT placement ngoài danh sách | Trả 400, báo vị trí không hợp lệ | N |

---

_Hết tài liệu — Software Design Description v1.4_
