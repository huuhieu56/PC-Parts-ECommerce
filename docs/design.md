...
| TC-AUTH-15 | Quên mật khẩu — gửi email thành công    | Account tồn tại với email test            | 1. POST /auth/forgot-password với email đúng                              | Trả 200, Reset Token tạo, email gửi liên kết đặt lại          | P    |
| TC-AUTH-16 | Quên mật khẩu — email không tồn tại     | Email không có trong DB                   | 1. POST /auth/forgot-password                                             | Trả 200, thông báo chung (không lộ email tồn tại hay không)    | N    |
| TC-AUTH-17 | Đặt lại mật khẩu — token hợp lệ         | Reset token chưa hết hạn                  | 1. POST /auth/reset-password với token + newPwd hợp lệ                    | Trả 200, password_hash cập nhật, token xóa, session cũ xóa    | P    |
| TC-AUTH-18 | Đặt lại mật khẩu — token hết hạn        | Reset token đã hết hạn                    | 1. POST /auth/reset-password                                              | Trả 400, "Liên kết đã hết hạn. Vui lòng yêu cầu lại"         | N    |
| TC-AUTH-19 | Cập nhật profile thành công              | Customer đã đăng nhập                     | 1. PUT /profile với fullName, phone, dateOfBirth mới                      | Trả 200, User/Profile cập nhật                                | P    |
| TC-AUTH-20 | Cập nhật profile — dữ liệu không hợp lệ | Customer đã đăng nhập                     | 1. PUT /profile với phone sai định dạng                                   | Trả 400, validation error                                     | N    |
| TC-AUTH-21 | Thêm địa chỉ mới                        | Customer đã đăng nhập                     | 1. POST /addresses với đầy đủ thông tin                                   | Trả 201, Address tạo thành công                                | P    |
| TC-AUTH-22 | Thêm địa chỉ — thiếu trường bắt buộc    | Customer đã đăng nhập                     | 1. POST /addresses thiếu province                                         | Trả 400, validation error                                     | N    |
| TC-AUTH-23 | Sửa địa chỉ                             | Address id=1 thuộc user                   | 1. PUT /addresses/1 với street mới                                        | Trả 200, Address cập nhật                                     | P    |
| TC-AUTH-24 | Đặt địa chỉ mặc định                    | User có ≥ 2 Address                       | 1. PATCH /addresses/2/default                                             | Trả 200, Address 2 is_default=true, Address cũ is_default=false | P    |
| TC-AUTH-25 | Xóa địa chỉ duy nhất                    | User chỉ có 1 Address (mặc định)          | 1. DELETE /addresses/1                                                    | Trả 400, "Bạn cần tạo địa chỉ mới trước khi xóa"             | N    |
| TC-AUTH-26 | Đăng nhập email sai định dạng            | —                                         | 1. POST /auth/login với email "not-an-email"                              | Trả 400, validation error                                     | N    |
| TC-AUTH-27 | Refresh Token hết hạn                    | Refresh token đã hết hạn                  | 1. POST /auth/refresh-token                                               | Trả 401, "Token đã hết hạn. Vui lòng đăng nhập lại"           | N    |
