...
| TC-AUTH-26 | Cập nhật profile thành công             | Customer đã đăng nhập                     | 1. PUT /profile với fullName, phone, dateOfBirth mới                      | Trả 200, User/Profile cập nhật                                | P    |
| TC-AUTH-27 | Cập nhật profile — dữ liệu không hợp lệ | Customer đã đăng nhập                     | 1. PUT /profile với phone sai định dạng                                   | Trả 400, validation error                                     | N    |
| TC-AUTH-28 | Thêm địa chỉ mới                        | Customer đã đăng nhập                     | 1. POST /addresses với đầy đủ thông tin                                   | Trả 201, Address tạo thành công                               | P    |
| TC-AUTH-29 | Thêm địa chỉ — thiếu trường bắt buộc    | Customer đã đăng nhập                     | 1. POST /addresses thiếu province                                         | Trả 400, validation error                                     | N    |
| TC-AUTH-30 | Sửa địa chỉ                             | Address id=1 thuộc user                   | 1. PUT /addresses/1 với street mới                                        | Trả 200, Address cập nhật                                     | P    |
| TC-AUTH-31 | Đặt địa chỉ mặc định                    | User có ≥ 2 Address                       | 1. PATCH /addresses/2/default                                             | Trả 200, Address 2 default=true, Address cũ default=false     | P    |
| TC-AUTH-32 | Xóa địa chỉ duy nhất                    | User chỉ có 1 Address (mặc định)          | 1. DELETE /addresses/1                                                    | Trả 400, "Bạn cần tạo địa chỉ mới trước khi xóa"              | N    |
| TC-AUTH-33 | Đăng nhập email sai định dạng           | —                                         | 1. POST /auth/login với email "not-an-email"                              | Trả 400, validation error                                     | N    |
| TC-AUTH-34 | Refresh Token hết hạn                   | Refresh token đã hết hạn                  | 1. POST /auth/refresh-token                                               | Trả 400/401, yêu cầu đăng nhập lại                            | N    |
